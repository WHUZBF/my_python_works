...
class Fraction:
    ...
    def update(self):
        '''更新数据'''
        self.str = self.sign + str(abs(int(self.numerator))) + '/' + str(abs(int(self.denominator)))  #使用int除去意外的小数因为整数相除就算是整除也会产生小数位数，这里不必考虑过多
        self.value = self.numerator / self.denominator
    ...
    def gcd(self):
        '''寻找分子分母的最大公约数'''
        '''辗转相除法'''
        num_1=abs(self.numerator)
        num_2=abs(self.denominator)    # 使用绝对值来计算公约数
        if num_1<num_2:
...
def transform(f):
    """将浮点数转化为分数"""
    len_f = len(str(f))-len(str(int(math.floor(f))))-1
    denominator = 10**len_f
    numerator = int(str(f).replace('.', ''))
